...
int main()
{
    ...
    // determina status e pontuação do jogo (se necessário) com base no primeiro lançamento dedados
    switch (sumOfDice)
    {
    ...
    while (gameStatus == CONTINUE) // nem WON nem LOST
    {
        ...
        if (sumOfDice == myPoint) // vitória por pontuação
            gameStatus = WON;
        else if (sumOfDice == 7) // perde obtendo 7 antes de atingir a pontuação
            gameStatus = LOST;
    } // fim do while
